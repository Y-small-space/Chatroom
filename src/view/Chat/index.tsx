...
export default function Chat() {
  ...
  const [selectUser_chat, setSelectUser_chat] = useState<number>(
    Number(userid) || 0
  );
  const allChatHistory = useSelector(
    (state: RootState) => state.allhistory.chatHistory
  );
  const [showHistory, setShowHistory] = useState<Message[]>([]);
    ...
    if (e.shiftKey && e.keyCode === 13) {
      ...
      if (typeof message === "string" && message.trim() !== "") {
        const messageObj: Message = {
          sender: userList._id,
          receiver: String(selectUser_chat),
          content: message,
          roomId: currentRoomId,
          sender_username: myId as string,
  ...
  // 拿到当前所选用户的roomId
  useEffect(() => {
    const roomid = sessionStorage.getItem(String(selectUser_chat));
