...
export default function UserCard(props) {
  const { selectUser_chat, setSelectUser_chat, user, chatHistory, showHistory } = props;
  ...
  useEffect(() => {
    ...
    if (selectUser_chat.phoneNumber !== user.phoneNumber) {
      ...
      const newChatHistory = showHistory.filter((message) => {
        return (
          (message.sender_username === String(myId) &&
            message.receiver_username === String(user.phoneNumber)) ||
          (message.sender_username === String(user.phoneNumber) &&
    ...
    <div>
      <Card
        style={{
          marginTop: "1rem",
          padding: "1%",
          backgroundColor: `${user.phoneNumber === selectUser_chat ? "transparent" : ""
            }`,
        }}
        key={user._id}
      ...
      >
        ...
        <div style={{ marginLeft: "20%" }}>
          ...
          <div style={{ lineHeight: "1.1rem", fontSize: "1.0rem", color: 'gray' }}>
            {currentHistory}
          </div>
        </div>
      </Card>
