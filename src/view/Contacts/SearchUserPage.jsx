...
export default function SearchUserPage() {
  ...
  const SearchUser = async () => {
    try {
      const response = await axios.get(`http://localhost:4000/api/search/${searchUser}`);
  ...
  return (
    ...
    >
      ...
      {searchResult &&
        <Card
          style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexDirection: 'column', width: '500px', marginTop: '20px', height: '280px', backgroundColor: '', textAlign: 'center', padding: '5%' }}
        >
          <Avatar size={60} src={`http://localhost:4000${searchResult.avatar}`} />
          ...
          <Button onClick={() => AddFriends(searchResult.phoneNumber)} style={{ marginTop: '20px', border: "0", backgroundColor: 'rgb(100,100,100,0.5)' }}>ADD</Button>
        </Card>}
    </Card>
  )
}
